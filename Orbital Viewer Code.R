...
#------------------------------------------------------------Testing area----------------------------------------------------------
# test_density_func <- hydrogen_prob_density_func(4,1,0)
# x = c(0,2)
# y = c(0,4)
# z = c(0,6)
# result =data.frame(x,y,z)
# result
# spher_coords <- matrix(cartesian_to_spherical(result$x,result$y,result$z),ncol = 3)
# spher_coords
# result <- cbind(spher_coords[,1],result)
# result <- cbind(spher_coords[,2],result)
# result <- cbind(spher_coords[,3],result)
# result[result == "NaN"] <- 0
# names(result)<-c("r","theta","phi")
# result
# output <- test_density_func(result$r,result$theta,result$phi)
# 
# output
# cartesian_to_spherical(-2.116709e-09,-1.646329e-09,-2.116709e-09)
# for (i in 0:101) {
#   progress(i)
#   Sys.sleep(0.01)
#   if (i == 101) cat("Done!\n")
